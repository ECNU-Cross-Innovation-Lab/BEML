...
category_dataset_path=Path("A:/ZQJ/ZQJ_CODEreporitory/productMining/embedding_results/embedding_labeled/datasetLabeled_GPC.csv")
result_dir=Path("A:/ZQJ/ZQJ_CODEreporitory/productMining/embedding_results/predict_result0214")
...
class ML_record():
    ...
    def save_record(self,name):
        path1=Path(result_dir,name+"score.csv")
        res_score = pd.DataFrame(self.score_dict)
        res_score.to_csv(path1)
    def save_record2dir(self,name,dir):
        path1 = Path(dir, name + "_score.csv")
        res_score = pd.DataFrame(self.score_dict)
        res_score.to_csv(path1)
    #        保存最终结果到目标文件
...
def dataset2category():
    ...
    ser_label_factorize=ser_label.factorize()
    unique=ser_label_factorize[1]
    ser_label_1=ser_label_factorize[0]
    return x,ser_label_1,unique
...
def save_model(name,model):
    with open('../model_saved/'+name+".pickle", 'wb')as f:
        pickle.dump(model,f)

def load_model(name):
    f = open('../model_saved/'+name+".pickle", 'rb')
    model=pickle.load(f)
    f.close()
    return model
