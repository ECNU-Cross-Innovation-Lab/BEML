...
class EntitySBERT(SentenceTransformer):
  ...
  def _pad_to_tensor(self, features) -> BatchEncoding:
    ...
    # print("打印batchtensor",batch_tensor)
  ...
  # 对输入进行编码，输出为一个联合类型（Union），可以是包含 PyTorch 张量的列表、NumPy 数组或单个 PyTorch 张量，具体取决于参数的设置和编码的输出格式。
  def encode(self,
    ...
    print("-1")
    # logger.info(f"exampleType: {type(examples[0])}")
    ...
    print("begin embedding and inference ")
    last_progress = 0
    for start_index in trange(0, len(examples), batch_size, desc="Batches", disable=not show_progress_bar):
      ...
      with torch.no_grad():
        ...
        # print("PrintOutfeatures",out_features)
        # print(type(out_features))
        # print("input_ids",out_features['input_ids'].shape)
        # print("attention_mask",out_features['attention_mask'].shape)
        # print("token_embeddings",out_features['token_embeddings'].shape)
        # print("pooling_mask",out_features['pooling_mask'].shape)
        #
