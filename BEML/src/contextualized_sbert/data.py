...
# 生成单个product的tokenize输出结果
def parse_tokenizer_product_output(tokenizer_output, max_seq_length, disable_tqdm=False):
  """Take tokenized output from sequence of"""
  # all_input_ids为[[float]]，记录tokenizer化后的结果
  all_input_ids = tokenizer_output.input_ids
  dataset = []
  n_combined=len(all_input_ids)
  # print("n_combined:",n_combined)
  for i in tqdm(range(n_combined), desc="Preprocess", disable=disable_tqdm):
    # 将实体的第一个和最后一个令牌放在序列的首尾.  确保输入序列的一致性
  ...
  # print("return dataset:",len(dataset))
  return dataset



# 生成编码后的单个例子的最后一步
# 生成不同entity的pooling mask
...
def parse_clf_tokenizer_output(entity_tokenizer_output, pt_tokenizer_output, max_seq_length, disable_tqdm=False):
  ...
  for i in tqdm(range(n_combined), desc="Preprocess", disable=disable_tqdm):
    ...
    attention_mask = [1] * len(input_ids) #用于区分待处理的id与后续padding加上的id
    # keep only entity repr minus special tokens
...
def preprocess_singleton_data(samples: List[EntityContext], tokenizer: PreTrainedTokenizer, max_seq_length=64, disable_tqdm=False):
  ...
  tokenizer_output = tokenizer(raw_sentences, truncation="do_not_truncate")
  # 从tokenizer后的结果中，筛选最终的
  dataset = parse_tokenizer_output(tokenizer_output, max_seq_length, disable_tqdm=disable_tqdm)
  return dataset


  # 传入待tokenizer化的str列表samples
  # 返回tokenize的结果
def preprocess_singleproduct_data(samples, tokenizer: PreTrainedTokenizer, max_seq_length=64, disable_tqdm=False):
  tokenizer_output = tokenizer(samples, truncation="do_not_truncate")
  dataset = parse_tokenizer_product_output(tokenizer_output, max_seq_length, disable_tqdm=disable_tqdm)
  return dataset
