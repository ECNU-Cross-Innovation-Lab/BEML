...
EXP_DIR=A:/ZQJ/ZQJ_CODEreporitory/OAMINE/OAMine-main/OAMine-main/value_grouping/out_put
...
for ((ITER=5;ITER<=$TRAIN_ITER;ITER++));
do
  PREV_ITER=$(($ITER-1))
  if (($ITER>1));
  then
...
#    如果不是最后一次，则使用该分类推理方法，再用得到的置信数据进一步优化模型
